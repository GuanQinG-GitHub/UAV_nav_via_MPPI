...
class OffboardFigure8Node(Node):
    ...
    def timer_callback(self) -> None:
        ...
        if(self.start_time + 100 > time.time()):
            self.publish_takeoff_setpoint(0.0, 0.0, self.altitude)
            self.get_logger().info("Taking off, hovering at 0.5 m")
        # elif(self.start_time + 20 > time.time() and self.start_time + 10 < time.time()):
        #     self.publish_takeoff_setpoint(0.2, 0.0, self.altitude + 0.3)
        #     self.get_logger().info("hovering at 0.2 m")
        elif(self.start_time + 120 > time.time() and self.start_time + 100 < time.time()):
            self.publish_takeoff_setpoint(0.0, 0.0, -0.2)
            self.get_logger().info("hovering at 0.2 m")
        else:
            self.land()
